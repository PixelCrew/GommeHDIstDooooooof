...
public enum ParticleEffect {
	...
	public void display(float d, float offsetY, float offsetZ, float speed,
		...
		new ParticleEffectPacket(this.name, d, offsetY, offsetZ, speed, amount)
				.sendTo(center, range);
	}
	...
	public void display(float offsetX, float offsetY, float offsetZ,
		...
		new ParticleEffectPacket(this.name, offsetX, offsetY, offsetZ, speed,
				amount).sendTo(center, players);
	}
	...
	public static void displayBlockCrack(int id, byte data, float offsetX,
		...
		new ParticleEffectPacket("blockcrack_" + id + "_" + data, offsetX,
				offsetY, offsetZ, 0.0F, amount).sendTo(center, range);
	}
	...
	public static void displayBlockCrack(int id, byte data, float offsetX,
		...
		new ParticleEffectPacket("blockcrack_" + id + "_" + data, offsetX,
				offsetY, offsetZ, 0.0F, amount).sendTo(center, players);
	}
	...
	public static final class ParticleEffectPacket {
		...
		public static void initialize()
			...
			try {
				int version = Integer.parseInt(Character
				...
				Class<?> packetClass = ReflectionUtils.PackageType.MINECRAFT_SERVER
						.getClass(version < 7 ? "Packet63WorldParticles"
								: ReflectionUtils.PacketType.PLAY_OUT_WORLD_PARTICLES
										.getName());
				...
				sendPacket = ReflectionUtils
						.getMethod(
								...
								"sendPacket",
								new Class[] { ReflectionUtils.PackageType.MINECRAFT_SERVER
										.getClass("Packet") });
		...
		public void sendTo(Location center, Player player)
				throws ParticleEffect.ParticleEffectPacket.PacketInstantiationException,
				ParticleEffect.ParticleEffectPacket.PacketSendingException {
			if (this.packet == null) {
				try {
					this.packet = packetConstructor.newInstance(new Object[0]);
					ReflectionUtils.setValue(this.packet, true, "a", this.name);
					ReflectionUtils.setValue(this.packet, true, "b",
							Float.valueOf((float) center.getX()));
					ReflectionUtils.setValue(this.packet, true, "c",
							Float.valueOf((float) center.getY()));
					ReflectionUtils.setValue(this.packet, true, "d",
							Float.valueOf((float) center.getZ()));
					ReflectionUtils.setValue(this.packet, true, "e",
							Float.valueOf(this.offsetX));
					ReflectionUtils.setValue(this.packet, true, "f",
							Float.valueOf(this.offsetY));
					ReflectionUtils.setValue(this.packet, true, "g",
							Float.valueOf(this.offsetZ));
					ReflectionUtils.setValue(this.packet, true, "h",
							Float.valueOf(this.speed));
					ReflectionUtils.setValue(this.packet, true, "i",
							Integer.valueOf(this.amount));
				} catch (Exception exception) {
			...
			try {
				sendPacket.invoke(playerConnection.get(getHandle.invoke(player,
						new Object[0])), new Object[] { this.packet });
			} catch (Exception exception) {
